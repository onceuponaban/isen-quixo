...
{
	glutTimerFunc(millisecondes, fonctionTimer, 0);
}
...
{
	static double tempsPrecedent = 0.;
	const double dureeImage = 1000./(double)imagesParSeconde;
	static double erreurTemporisation = 0.;
	double tempsCourant;
	int temporisation;
	
	if (imagesParSeconde > 100) imagesParSeconde = 100;
	else if (imagesParSeconde <= 0) imagesParSeconde = 1;
	
	rafraichisFenetre(); // On demande le rafraichissement quoi qu'il arrive
	tempsCourant = tempsReel();

	if (tempsPrecedent != 0.)
	{
		erreurTemporisation += (tempsCourant-tempsPrecedent)-dureeImage*.001; // Le delta
																			  // de temps indique l'erreur de temporisation commise
	}
	// On redemarre la callback Timer
	temporisation = (int)(dureeImage-erreurTemporisation*1000.);
	glutTimerFunc(temporisation >= 0 ? (unsigned int)temporisation : 0U, demandeAnimation_ips, imagesParSeconde);

	tempsPrecedent = tempsCourant; // On memorise l'heure de lancement
		// du message Affichage
}
...
static void fonctionClavierSpecial(int codeTouche, int xSouris, int ySouris)
{
	/* On memorise la touche et la position graphique (et pas fenetre) de la souris pour pouvoir les fournir par la suite */
	switch
		(codeTouche)
	{
		case GLUT_KEY_F1:
